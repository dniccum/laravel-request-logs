...
it('can remove logs that are older than 3 weeks', function () {
    ...
    RequestLog::factory($numberOfRecordsToCreate)->create();

    $date = now()->subDays(config('request-logs.history'));
    $numberOfRecords = RequestLog::where('request_start', '<', $date)
        ->count();

    \Pest\Laravel\artisan('request-logs:clean')->assertOk();

    $leftOverRecords = RequestLog::where('request_start', '>', $date)
        ->count();
