...
it('can remove logs that are older than 3 weeks', function () {
    ...
    $numberOfRecords = \Dniccum\LaravelRequestLogs\Models\RequestLog::where('created_at', '<', $date)
        ->count();

    \Pest\Laravel\artisan('request-logs:clean')->assertOk();

    $leftOverRecords = \Dniccum\LaravelRequestLogs\Models\RequestLog::where('created_at', '>', $date)
        ->count();
